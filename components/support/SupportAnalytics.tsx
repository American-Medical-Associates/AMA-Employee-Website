...
const SupportAnalytics: React.FC<{
  selectedDate: any
  supportTickets: any
}> = ({ selectedDate, supportTickets }) => {
  const [supportAnalyticsArray, setSupportAnalyticsArray] = useState<
    Array<any>
  >([])
  ...
  const [numberOfSupportTickets, setNumberOfSupportTickets] = useState(true)
  ...
  useEffect(() => {
    return () => {
      GetSupportTickets({ supportTicketsState: setSupportAnalyticsArray })
    }
  ...
  if (day) {
    ...
    data = daysInMonthArray.map((day: any) => {
      ...
      supportAnalyticsArray.map((supportTracking) => {
        if (
          supportTracking.timestamp.toDate().getDate() == day &&
          supportTracking.timestamp.toDate().getMonth() + 1 ==
            selectedDate.getMonth() + 1
        ) {
  ...
  if (month) {
    ...
    data = monthsInYear.map((month: any) => {
      ...
      supportAnalyticsArray.map((supportTracking) => {
        if (
          supportTracking.timestamp.toDate().getMonth() + 1 == month &&
          supportTracking.timestamp.toDate().getFullYear() ==
  ...
  if (all) {
    ...
    data = yearsTotal.map((year: any) => {
      ...
      //1.get the number of spravatos for every day in the month and display it on the graph
      supportAnalyticsArray.map((supportTracking) => {
        if (supportTracking.timestamp.toDate().getFullYear() == year) {
          monthNumber = monthNumber + 1
          totalDayNumber = totalDayNumber + 1
        }
      })
        ...
        <div className="flex h-[80vh] flex-col">
          <div className="flex flex-row p-10 ">
            ...
            <div className="flex w-[75%] flex-col items-center justify-center">
              {numberOfSupportTickets && data != undefined && data != null && (
                //@ts-ignore
                <BarGraph
                  data={data}
                  day={day}
