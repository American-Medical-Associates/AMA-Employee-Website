...
export async function addNewPatient({
  ...
  await setDoc(
    doc(db, 'companys', company, 'patients', email.trim().toLowerCase()),
    {
...
export async function submitNewPatientPacketAndCreateNewPatient({
  ...
  await setDoc(
    ...
    .then(async () => {
      await setDoc(
        doc(db, 'companys', 'AMA', 'patients', emailValue.trim().toLowerCase()),
        {
          fullName:
...
export async function AddPictureOfPatientFaceToStorageAndToDB({
  ...
  const imageRef = ref(
    storage,
    `NewPatientPacket/${emailValue.trim().toLowerCase()}/patientFace`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export async function AddPictureOfPatientInsuranceToStorageAndToDB({
  ...
  const imageRef = ref(
    storage,
    `NewPatientPacket/${emailValue.trim().toLowerCase()}/PrimaryInsuranceCard`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export async function AddPictureOfPatientInsuranceBackToStorageAndToDB({
  ...
  const imageRef = ref(
    storage,
    `NewPatientPacket/${emailValue
      .trim()
      .toLowerCase()}/PrimaryInsuranceCardBack`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export async function AddPictureOfPatientInsuranceSecondaryToStorageAndToDB({
  ...
  const imageRef = ref(
    ...
    `NewPatientPacket/${emailValue
      .trim()
      .toLowerCase()}/SecondaryInsuranceCard`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export async function AddPictureOfPatientInsuranceSecondaryBackToStorageAndToDB({
  ...
  const imageRef = ref(
    ...
    `NewPatientPacket/${emailValue
      .trim()
      .toLowerCase()}/SecondaryInsuranceCardBack`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export async function AddPictureOfDriverLicenseToStorageAndToDB({
  ...
  const imageRef = ref(
    storage,
    `NewPatientPacket/${emailValue.trim().toLowerCase()}/DriverLicense`,
  )
  await uploadString(imageRef, selectedFile, 'data_url').then(
    async (snapshot) => {
      const download = await getDownloadURL(imageRef)
...
export function GetPatientInfo({ email, setPatientInfo }) {
  onSnapshot(
    doc(db, 'companys', 'AMA', 'patients', email.trim().toLowerCase()),
    (doc) => {
...
export async function SubmitWeightLossSurvey({
  ...
  await setDoc(
    ...
    await setDoc(
      doc(db, 'companys', 'AMA', 'WeightLoss', emailValue.trim().toLowerCase()),
      {
        emailValue: emailValue.trim().toLowerCase(),
        patientsName: patientsName,
        date: date,
...
export async function submitMadrs({
  name,
  date,
  dob,
  totalScore,
  ...
  try {
    ...
    const submissionId = doc(collection(db, 'companys', 'AMA', 'MADRS')).id

    // Create a document reference
    const docRef = doc(db, 'companys', 'AMA', 'MADRS', submissionId)
    ...
    await setDoc(docRef, {
      ...
      apparentSadness: apparentSadness,
      reportedSadness: reportedSadness,
      innerTension: innerTension,
      reducedSleep: reducedSleep,
      reducedAppetite: reducedAppetite,
      concentrationDifficulties: concentrationDifficulties,
      lassitude: lassitude,
      inabilityToFeel: inabilityToFeel,
      pessimisticThoughts: pessimisticThoughts,
...
export function GetMadrs({ setMadrs }) {
  try {
    const q = query(collection(db, 'companys', 'AMA', 'MADRS'))
    onSnapshot(q, (querySnapshot) => {
      const submissions = []
      querySnapshot.forEach((doc) => {
        submissions.push({ id: doc.id, ...doc.data() })
      })
      setMadrs(submissions)
    })
  } catch (error) {
    alert(error)
  }
...
// Firebase function to get a specific MADRS submission
export async function getSpecificMadrs(id) {
  const docRef = doc(db, 'companys', 'AMA', 'MADRS', id)
...
export async function submitAims({
  patientName,
  date,
  dateOfBirth,
  rater,
  answers, // Include answers in the parameter destructuring
  submissionTime,
}) {
  const submissionId = doc(collection(db, 'companys', 'AMA', 'AIMS')).id
  const docRef = doc(db, 'companys', 'AMA', 'AIMS', submissionId)
  ...
  await setDoc(docRef, {
    ...
    dateOfBirth,
    rater,
    answers, // Include answers in the data to be saved
    id: submissionId,
    submissionTime,
  })
}

export async function getSpecificAims(id) {
  const docRef = doc(db, 'companys', 'AMA', 'AIMS', id)
  const docSnap = await getDoc(docRef)
...
export async function submitControlledSubstance({
  patientId, // Assuming patientId is being passed when calling this function
  patientName,
  dateOfBirth,
  receivedSubstances,
  pharmacy,
  ...
  // Create a new document reference with a unique ID in the specified collection
  ...
  await setDoc(docRef, {
    ...
    amaEmployeeInitials,
    id: submissionId,
    patientId, // Including patientId in the document for reference
  })
}

export async function submitRecordsRequest({
  patientId,
  patientName,
  dateOfBirth,
  facility,
  ...
  const docRef = doc(db, 'companys', 'AMA', 'Records Request', submissionId)
...
export async function submitCancellationPolicy({
  patientId,
  patientName,
  dateOfBirth,
  signature,
  ...
  const docRef = doc(db, 'companys', 'AMA', 'Cancellation Policy', submissionId)
  await setDoc(docRef, {
    patientId,
    patientName,
    dateOfBirth,
...
export async function submitPhentermineForm({
  patientId, // Assuming patientId is being passed when calling this function
  patientName,
  dateOfBirth,
  initials,
...
export async function submitExcisionForm({
  patientId, // Assuming patientId is being passed when calling this function
  patientName,
  providersName,
  initials,
  ...
  // Create a new document reference with a unique ID in the specified collection
  const submissionId = doc(
    collection(db, 'companys', 'AMA', 'Excision Form'),
  ).id
  const docRef = doc(db, 'companys', 'AMA', 'Excision Form', submissionId)
...
export async function submitPhqForm({
  name,
  date,
  totalScore,
  littleInterest,
  ...
  try {
    // Generate a unique submissionId
    const submissionId = doc(collection(db, 'companys', 'AMA', 'PHQ Form')).id

    // Create a document reference
    const docRef = doc(db, 'companys', 'AMA', 'PHQ Form', submissionId)
...
export function GetPhq({ setPhq }) {
  try {
    const q = query(collection(db, 'companys', 'AMA', 'PHQ Form'))
    onSnapshot(q, (querySnapshot) => {
      const submissions = []
      querySnapshot.forEach((doc) => {
        submissions.push({ id: doc.id, ...doc.data() })
...
// Firebase function to get a specific MADRS submission
export async function getSpecificPhq(id) {
  const docRef = doc(db, 'companys', 'AMA', 'PHQ Form', id)
  const docSnap = await getDoc(docRef)
...
export async function submitPatientReleaseForm(props) {
  const submissionId = doc(
    collection(db, 'companys', 'AMA', 'releaseProtectedHealthInformation'),
  ).id
  const docRef = doc(
    db,
    'companys',
  ...
  await setDoc(docRef, {
    ...
    lastFourSSN: props.lastFourSSN,
    patientStreetAddress: props.patientStreetAddress,
    patientCity: props.patientCity,
    patientState: props.patientState,
    patientZip: props.patientZip,
    patientPhone: props.patientPhone,
    purposeOfRelease: props.purposeOfRelease,
    treatmentFrom: props.treatmentFrom,
    treatmentTo: props.treatmentTo,
    facilityName: props.facilityName,
    facilityAddress: props.facilityAddress,
    facilityPhone: props.facilityPhone,
    facilityFax: props.facilityFax,
    nameOfPractice: props.nameOfPractice,
    addressOfPractice: props.addressOfPractice,
    practicePhone: props.practicePhone,
    practiceFax: props.practiceFax,
    hospital: props.hospital,
    officeClinic: props.officeClinic,
    format: props.format,
    deliveryMethod: props.deliveryMethod,
    patientSignature: props.patientSignature,
    legalGuardianSignature: props.legalGuardianSignature,
    relationshipToPatient: props.relationshipToPatient,
    dateOfRequest: props.dateOfRequest,
    date: props.date,
  })
}
