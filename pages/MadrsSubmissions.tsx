...
const MadrsSubmissions = () => {
  ...
  const orderedMadrs = madrs
    ...
    .filter(
      (submission) =>
        submission.name.includes(searchTerm) ||
  ...
  const currentSubmissions = orderedMadrs.slice(
      ...
      <div className="flex flex-col items-center justify-center mt-8">
        ...
        <div>
          {currentSubmissions.map((submission) => (
            ...
            >
              <p>
                {submission.name} - {submission.date}
              </p>
            </div>
          ))}
        </div>
        <div className="flex mt-4">
          {[
            ...Array(
              Math.ceil(orderedMadrs.length / submissionsPerPage),
            ).keys(),
          ].map((number) => (
            <button
              key={number}
